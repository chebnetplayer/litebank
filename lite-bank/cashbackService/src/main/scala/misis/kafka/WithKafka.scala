...
trait WithKafka {
  ...
  def kafkaSource[T](implicit decoder: Decoder[T], topicName: TopicName[T]) = Consumer
    ...
    .log(s"Ошёибка во время чтения топика ${topicName.get}")
    .collect {
      case Right(command) =>
        command
      case Left(error) => throw new RuntimeException(s"Ошибка при чтении сообщения $error")
    }
  ...
  def kafkaSink[T](implicit encoder: Encoder[T], topicName: TopicName[T]) = Flow[T]
    ...
    .log(s"Ошибка при обработке сообщения топика ${topicName.get}")
  ...
  def produceCommand[T](command: T)(implicit encoder: Encoder[T], topicName: TopicName[T]) = {
    Source
      .single(command)
      .map { command =>
        println(s"Отправка сообщения $command в топик ${topicName.get}")
